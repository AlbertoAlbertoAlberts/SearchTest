...
export async function extractPricesOnly(
  ...
  const maxPages = Math.min(Math.ceil(maxResults / itemsPerPage), 6); // Scan up to 6 pages (288 items)
  
  console.log(`[Andele] Will scan ${maxPages} pages (${maxPages * itemsPerPage} potential items)`);
  
  try {
    // Fetch pages in batches of 5 for better parallelization
    const BATCH_SIZE = 5;
      ...
      const batchPromises = pages.map(async (page) => {
        ...
        try {
          ...
          const html = await fetchWithBrowser(url, {
            ...
            waitFor: 800 // Reduced wait time for faster response
          });
