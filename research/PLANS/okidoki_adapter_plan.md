# Okidoki.ee Adapter Implementation Plan

**Date:** 29 December 2025  
**Target marketplace:** Okidoki.ee (Estonia)  
...
**Proposed source key:** `okidoki`

**File path:** `lib/adapters/okidoki.js`

**Reason for key choice:**
- Matches the actual site domain name
- Short and memorable
...
## 2. How Okidoki.ee works technically
...
4. **Check for protection mechanisms**
   - Test if Cloudflare or similar protection is in place
   - Verify if simple HTTP requests work or if browser automation is needed

### Expected outcome (to be verified during implementation)

Based on typical Estonian marketplace patterns:
- **Most likely:** Server-rendered HTML with results in initial page load (like SS.lv)
- **Alternative:** JS-rendered with Cloudflare protection (like Osta.ee)
- **Possible:** Hybrid approach (initial results server-rendered, lazy-loading more via JS)
...
```
https://www.okidoki.ee/search?q={query}
OR
https://www.okidoki.ee/otsing?q={query}
OR
...
- Example: `https://www.okidoki.ee/search?q=iphone&page=2`
...
```
https://www.okidoki.ee/search?q={query}&sort=price_asc
https://www.okidoki.ee/search?q={query}&sort=price_desc
https://www.okidoki.ee/search?q={query}&order=price
https://www.okidoki.ee/search?q={query}&orderBy=price&orderDir=asc
```
...
```
https://www.okidoki.ee/search?q={query}&price_min=50&price_max=500
https://www.okidoki.ee/search?q={query}&minPrice=50&maxPrice=500
```

**Alternative parameter names:**
- `priceFrom`, `priceTo`
- `min_price`, `max_price`
- `hind_min`, `hind_max` (Estonian: "hind" = price)
...
- `source` — always `"okidoki"`

**Return format:**
```javascript
return [
  { url: "https://...", priceText: "45 €", priceValue: 45, currency: "EUR", imageUrl: "https://...", source: "okidoki" },
  { url: "https://...", priceText: "120 €", priceValue: 120, currency: "EUR", imageUrl: "https://...", source: "okidoki" },
...
export async function enrichDetails(urls) {
  ...
  // e.g., ["https://www.okidoki.ee/item/123", "https://www.okidoki.ee/item/456"]
}
```
...
### Mapping table (Okidoki.ee → SPEC.md Listing)

| **SPEC.md field**      | **Okidoki.ee source**                       | **Extraction notes**                                                 |
|------------------------|---------------------------------------------|----------------------------------------------------------------------|
| `id`                   | Hash of `source + url`                      | Generated in `normalize.js`                                          |
| `source`               | Hardcoded: `"okidoki"`                      | Adapter responsibility                                               |
| `sourceName`           | Hardcoded: `"Okidoki.ee"`                   | Adapter responsibility                                               |
...
| `priceValue`           | Parsed from `priceText`                     | Use local `parsePriceValue()` helper; Infinity for unparseable      |
...
### Fields likely UNAVAILABLE on Okidoki.ee (based on typical Estonian marketplaces)
...
**Decision point:** Test during implementation with real requests in Step 1.
...
2. Log warnings when primary selectors fail:
   ```javascript
   if (!title) {
     console.warn(`[Okidoki] No title found for ${url}`);
   }
   ```
...
3. **Error logging:**
   ...
   try {
     ...
     console.error('[Okidoki] Failed to fetch:', error.message);
...
### Test script: `test-okidoki-integration.js`

**Purpose:** Verify adapter works end-to-end before integrating with orchestrator

**Location:** `test-okidoki-integration.js` (root of project)
...
2. **Test 2: Results have required fields**
   ...
   results.forEach(item => {
     ...
     assert(item.source === 'okidoki', "Source should be 'okidoki'");
   });
...
**Usage:**
```bash
node test-okidoki-integration.js
```
...
## 9. Lessons learned from Osta.ee

### What went wrong with Osta.ee

1. **All URL patterns returned 404 errors** - site may be offline, restructured, or fully blocking automation
2. **Cloudflare protection was too aggressive** - even Puppeteer couldn't bypass it
3. **No fallback strategy** - we didn't have alternative Estonian marketplaces planned

### Improvements for Okidoki.ee

1. **Test accessibility FIRST** (Step 1)
   - Before writing any code, verify the site is accessible
   - Test multiple URL patterns immediately
   - Check if simple HTTP requests work before committing to Puppeteer

2. **Have a fallback plan**
   - If Okidoki.ee also blocks us, move to Soov.ee
   - Keep a list of alternative Estonian marketplaces ready

3. **Start simple, add complexity if needed**
   - Try `fetchHtml()` first (fast)
   - Only use Puppeteer if necessary
   - Don't assume we need heavy tools upfront

4. **Document URL patterns early**
   - Save HTML samples for inspection
   - Document working URL patterns immediately
   - Test pagination and filters before implementing

---

## 10. Next 5 concrete implementation steps

### Step 1: Site reconnaissance (research phase) — CRITICAL
**Time estimate:** 20-30 minutes  
**Tasks:**
- Open https://www.okidoki.ee in browser
- **Verify site is accessible** (not 404, not blocked)
- Search for test query: "iphone"
- Document the actual URL after performing search
- Check if simple `curl` or `fetch()` works (no Cloudflare?)
...
- Test at least 3 different URL patterns before proceeding

**Output:** 
- Accessibility confirmation (✅ site works / ❌ site blocked)
- Notes file with URL patterns and rendering method decision
- HTML sample saved for selector inspection

**Success criteria:**
- ✅ Site returns actual search results (not 404)
- ✅ At least one working URL pattern documented
- ✅ Rendering method determined (server vs JS)

**If this step fails:** Skip to Soov.ee or another Estonian marketplace
...
- Import necessary dependencies:
  ...
  - `getDescriptionPreview` from `lib/textHelpers.js`
...
**Tasks:**
- Build search URL generator function using verified patterns from Step 1
- Implement page fetching loop (fetch pages 1, 2, 3... until `maxResults`)
- Parse search result items using CSS selectors discovered in Step 1
  - Listing URL
  - Price text
  - Thumbnail URL (optional)
- Convert price text to numeric value (Estonian format support)
...
- **Test immediately with real query**

**Output:** Working `scanPrices` function that returns price-sorted URLs

**Test command:**
```bash
node -e "import('./lib/adapters/okidoki.js').then(m => m.scanPrices('iphone', {maxResults: 20}).then(console.log))"
```

**Success criteria:**
- Returns array with at least 10 results for common query
- Each result has: url, priceText, priceValue, source
...
- **Test with real URLs from scanPrices**

**Output:** Working `enrichDetails` function

**Test command:**
```bash
node -e "import('./lib/adapters/okidoki.js').then(m => m.enrichDetails(['https://www.okidoki.ee/item/...', 'https://www.okidoki.ee/item/...']).then(console.log))"
```

**Success criteria:**
- Returns enriched data for at least 80% of input URLs
- Each result has: title, url, description (best effort)
...
**Tasks:**
- Create `test-okidoki-integration.js` in project root
...
- Run test script: `node test-okidoki-integration.js`
...
**Success criteria:**
- All 5 tests pass
- Adapter returns consistent data structure
- No crashes on edge cases (empty results, invalid query, network errors)

---

## 11. Post-implementation: Integration with orchestrator

Once Steps 1-5 are complete and tests pass:

**Step 6: Register adapter in API orchestrator**
- Open `pages/api/search.js`
- Add import: `import * as okidokiAdapter from "../../lib/adapters/okidoki.js";`
- Add to `ADAPTERS` map: `okidoki: okidokiAdapter,`
- No other changes needed (orchestrator already supports new sources)

**Step 7: Update UI to show Okidoki.ee**
- Add "Okidoki.ee" checkbox to source selector in `pages/index.js`
- Verify search works end-to-end with multiple sources
- Test with SS.lv and Andele to ensure no regressions

**Step 8: Production readiness**
- Add Okidoki.ee to README.md list of supported sources
...
|-----------------------------------|----------------|------------|--------------------------------------------------------------------------------|
| Site is inaccessible (like Osta)  | Medium         | High       | Test accessibility in Step 1 BEFORE writing code; have Soov.ee as backup     |
| Site is JS-rendered (slow)         | Medium         | Medium     | Use Puppeteer with browser caching; accept slower speed as necessary tradeoff |
| Cloudflare blocks automation       | Medium         | High       | Use Puppeteer with realistic fingerprints; add delays; test early             |
...
✅ Site is accessible and returns search results (verified in Step 1)  
...
✅ Performance is acceptable (< 5 seconds for first page with 20 results)
...
| **Phase**                  | **Time estimate** |
|----------------------------|-------------------|
| Step 1: Reconnaissance     | 20-30 minutes     |
...
**Note:** If Step 1 reveals site is inaccessible, stop immediately and move to Soov.ee (saves 3-4 hours)

---

## 15. Appendix: Estonian language terms

### Common Estonian words to recognize in HTML/selectors

| **Estonian**    | **English**       | **Usage**                              |
|-----------------|-------------------|----------------------------------------|
| kuulutus        | advertisement     | Listing/ad                             |
| otsing          | search            | Search page                            |
| hind            | price             | Price field                            |
| kokkuleppel     | negotiable        | Price by agreement                     |
| tasuta          | free              | Free items                             |
| seisukord       | condition         | Item condition                         |
| uus             | new               | New condition                          |
| kasutatud       | used              | Used condition                         |
| asukoht         | location          | Location/city                          |
| linn            | city              | City                                   |
| päeva tagasi    | days ago          | Relative date                          |
| täna            | today             | Today                                  |
| eile            | yesterday         | Yesterday                              |

---

## 16. Appendix: Example code patterns

### URL builder example
```javascript
function buildSearchUrl(query, page = 1, sortBy = 'price-low') {
  // Base pattern from Step 1 reconnaissance
  let url = `https://www.okidoki.ee/search?q=${encodeURIComponent(query)}`;
...
### Price parser example (Estonian format)
```javascript
function parseOkidokiPriceValue(priceText) {
  if (!priceText || typeof priceText !== 'string') return Infinity;
  
  const text = priceText.toLowerCase().trim();
  
  // Handle free listings (Estonian: "tasuta")
...
### Selector strategy example
```javascript
function extractPrice($item) {
  // Try multiple possible selectors
  const priceText = 
    $item.find('.price').text().trim() ||
    $item.find('[data-price]').text().trim() ||
    $item.find('.listing-price').text().trim() ||
    $item.find('[class*="price"]').first().text().trim() ||
    $item.find('[itemprop="price"]').text().trim() || // Schema.org
    '';
  
  if (!priceText) {
    console.warn('[Okidoki] No price found for item');
  }
...
**End of plan. Ready for Step 1: Reconnaissance.**

**IMPORTANT:** Begin with Step 1 to verify site accessibility before proceeding with implementation. If Okidoki.ee is inaccessible (like Osta.ee), document findings and move to Soov.ee instead.
