...
export async function scanPrices(query, options = {}) {
  ...
  try {
    ...
    console.error('[SS.com] scanPrices error:', error);
    return [];
  }
}
...
export async function enrichDetails(urls) {
  ...
  console.log(`[SS.com] enrichDetails: ${urls.length} URLs`);
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (url) => {
        try {
          ...
          console.warn(`[SS.com] Failed to enrich ${url}:`, error.message);
          failedCount++;
          return null;
        }
      })
  ...
  if (failedCount > 0) {
    console.warn(`[SS.com] enrichDetails: Failed ${failedCount} out of ${urls.length} URLs`);
  }

  console.log(`[SS.com] enrichDetails: Successfully enriched ${enriched.length} items`);
  return enriched;
}
